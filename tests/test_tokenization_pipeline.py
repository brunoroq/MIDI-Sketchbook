...
_STAGE_ONE_CONFIGURATION = {"fixture": "stage-one-schema-two"}
...
def test_pipeline_publishes_all_splits_with_bounded_special_tokens(
    ...
    assert manifest["preprocessing"]["schema_version"] == 2
...
def test_fragment_changed_during_encoding_is_detected(
    ...
    def mutating_encode(tokenizer, path):
        result = real_encode(tokenizer, path)
